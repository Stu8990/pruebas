...
| Styling | `styles.css`, tokens on `:root`, dark mode via `prefers-color-scheme` |
| Tests | `npm test` (node:test, pure logic) · `npx playwright test` (E2E with mocked Supabase, mobile + desktop, axe) |
