...
| Styling | `styles.css`, tokens on `:root`. Three palettes chosen in Más → Estilo (`html[data-palette]`: jacaranda default, ambar, guayaba; each sets colors, fonts, radii). Light by default; Tema: Claro/Oscuro/Automático |
...
- The auth hero background is a CSS gradient built from palette tokens (no image) until a palette is chosen. Icons in `icons/` (SVG + PNG).
